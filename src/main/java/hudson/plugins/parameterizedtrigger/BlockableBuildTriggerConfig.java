...
public class BlockableBuildTriggerConfig extends BuildTriggerConfig {
    ...
    @Override
    public List<QueueTaskFuture<? extends AbstractBuild>> perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
        List<QueueTaskFuture<? extends AbstractBuild>> r = super.perform(build, launcher, listener);
    ...
    protected QueueTaskFuture schedule(AbstractBuild<?, ?> build, Job project, List<Action> list) throws InterruptedException, IOException {
        if (block!=null) {
            while (true) {
                ...
                QueueTaskFuture f = schedule(build, project, 0, list);
